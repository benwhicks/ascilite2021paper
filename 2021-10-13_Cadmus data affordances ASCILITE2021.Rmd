...
cadmus_session_df_rel <- cadmus_session_df %>% 
  ...
  inner_join(
    cadmus_summary_df_rel %>% 
      distinct(assessment_id, subject_name, subject_id, 
               assessment_name, grade, mark, work_id, 
               user_id, due_date, offline_work, usage)) %>% 
...
n_students_submitting_work <- nrow(distinct(cadmus_summary_df_rel, id))
n_assessments_submitted <- nrow(distinct(cadmus_summary_df_rel, assessment_id, id))
n_sessions <- nrow(cadmus_session_df_rel)

```

## Overall usage

`r n_students_submitting_work` students submitted some assessment, for a total of `r n_assessments_submitted` distinct pieces of work. This was completed over `r n_sessions` separate log in sessions.
...
dat_resources %>% 
  ...
  geom_histogram(aes(n_resources_accessed, fill = assessment_grade), 
                 binwidth = 1, color = "black", position = "fill") +
...
dat_feedback <- cadmus_summary_df_rel %>% 
  ...
  mutate(
    mark_scaled_2sd = relative_mark / 2
  ) %>% 
  ungroup()

dat_comp <- dat_feedback %>% 
  select(assessment_name, user_id, work_id, relative_mark, 
         mark_scaled_2sd, viewed_previous_feedback) %>% 
  inner_join(
    dat_resources %>% 
...
# ggsave('ascilite2021-cadums-figure-1.png', plot = figure_1 +
#   theme(
#     text = element_text(size = 16),
#     panel.border = element_rect(color = "black", fill = NA, size = 1)), dpi = 400)
```
...
Modelling focuses on users working on the assignment 'in tool' - without a high percentage of pasting. 

``` {r }
library(brms)
library(bayesplot)
# only on 'in tool' users
intool.data <- dat_comp %>% 
  filter(complete.cases(.), usage == "in tool") 

all.data <- dat_comp %>% 
  filter(complete.cases(.)) 

clicks_sd <- sd(intool.data %>% pull(clicks_prior_raw), na.rm = T)

mod_form <- formula(relative_mark ~ viewed_previous_feedback + accessed_resources + start_early + clicks_prior_2sd)

intool.data <- intool.data %>% 
  mutate(clicks_prior = clicks_prior_raw / clicks_sd) %>% 
  mutate(clicks_prior_2sd = clicks_prior_raw / (2 * clicks_sd))

mod.pp <- brm(
  relative_mark ~ viewed_previous_feedback + accessed_resources + start_early + clicks_prior_2sd,
  prior = prior(normal(0, 1)),
  sample_prior = "only",
  data = intool.data)

pp.intervals.data <- mcmc_intervals_data(mod.brms.intool.pp)
...
mcmc_intervals(mod.pp)


figure_2 <- mcmc_intervals(
  mod, 
  regex_pars = "^b_[vasc]", 
  ...
  ggtitle("Comparative early indicators for assessment performance", 
          subtitle = "Showing point estimate with 50% and 90% compatability intervals")
  
figure_2


ggsave('ascilite2021-cadmus-figure-2.png', 
       plot = figure_2, 
       dpi = 400)
